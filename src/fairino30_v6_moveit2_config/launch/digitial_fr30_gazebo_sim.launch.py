...
def generate_launch_description():
    pkg_share = get_package_share_directory('fairino30_v6_moveit2_config')
    ...
    fairino30_controller = TimerAction(
        period=5.0,
        actions=[
            Node(
                package='controller_manager',
                executable='spawner',
                arguments=['fairino30_controller', '--controller-manager', '/controller_manager'],
                output='screen'
            )
        ]
    )
    ...
    kinematics_yaml = os.path.join(
        get_package_share_directory('fairino30_v6_moveit2_config'),
        'config',
        'kinematics.yaml'
    )
    ...
    static_virtual_joint_tfs = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            PathJoinSubstitution([
                FindPackageShare('fairino30_v6_moveit2_config'),
                'launch',
