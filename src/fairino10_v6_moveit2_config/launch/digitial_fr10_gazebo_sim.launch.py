...
def generate_launch_description():
    pkg_share = get_package_share_directory('fairino10_v6_moveit2_config')
    ...
    fairino10_controller = TimerAction(
        period=5.0,
        actions=[
            Node(
                package='controller_manager',
                executable='spawner',
                arguments=['fairino10_controller', '--controller-manager', '/controller_manager'],
                output='screen'
            )
        ]
    )
    ...
    static_virtual_joint_tfs = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            PathJoinSubstitution([
                FindPackageShare('fairino10_v6_moveit2_config'),
                'launch',
