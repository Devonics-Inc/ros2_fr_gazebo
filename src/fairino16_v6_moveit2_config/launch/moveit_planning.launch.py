...
def generate_launch_description():
    pkg_share = FindPackageShare('fairino16_v6_moveit2_config').find('fairino16_v6_moveit2_config')

    # Load robot description (same URDF as Gazebo)
    xacro_file = os.path.join(pkg_share, 'config', 'fairino16_v6_robot.urdf.xacro')
    ...
    robot_description_semantic = {
        "robot_description_semantic": PathJoinSubstitution(
            [pkg_share, "config", "fairino16_v6.srdf"]
        )
    }
    ...
    moveit_test_node = Node(
        package="fairino16_v6_moveit2_config",
