...
def generate_launch_description():
    pkg_share = get_package_share_directory('fairino20_v6_moveit2_config')
    ...
    fairino20_controller = TimerAction(
        period=5.0,
        actions=[
            Node(
                package='controller_manager',
                executable='spawner',
                arguments=['fairino20_controller', '--controller-manager', '/controller_manager'],
                output='screen'
            )
        ]
    )
    ...
    kinematics_yaml = os.path.join(
        get_package_share_directory('fairino20_v6_moveit2_config'),
        'config',
        'kinematics.yaml'
    )
    ...
    static_virtual_joint_tfs = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            PathJoinSubstitution([
                FindPackageShare('fairino20_v6_moveit2_config'),
                'launch',
